...
> **🆕 2026-06-10 本視窗摘要（訊息雙向打通／演出頁大改／一堆手機修正｜務必先讀 §14）**：①🔴**訊息客戶↔樂手同一聊天室即時雙向**（修掉 3 個疊在一起的雷，最深的是「建立對話前 getDoc 讀不存在的 doc 被規則擋」）；樂手後台「訊息」改讀/回 `conversations`（**查詢一定要 `participants array-contains`，不能 `musicianUid==`，否則整個查詢被拒**）；`conversations` 規則業主已發布。②客戶/樂手身分隔離 `opusz_role`。③nav「My Account」全站一致（**別加 `#navAccountBtn{display:none}`**，見 §7 9a）。④演出頁：日期搬上方 eyebrow、曲目/說明拆開（說明可展開、手機改彈卡）、Hero 標題在 MacBook/iPad/手機顯示、**後台新增 Banner 濾鏡暗度滑桿**（檔案型、localhost 調＋發布）。⑤手機：樂手後台漢堡抽屜、聊天泡泡修正、首頁 Hero 捲動手機只載 1/4 影格(效能)。⑥**後台 Firebase 登入會過期**（症狀：Failed to load customers）→ 重新登入。詳見 §14。
...
>
> **2026-06-09 本視窗摘要（樂手審核制 ＋ 客戶真帳號 ＋ 平台政策 ＋ PWA ＋ 修頭貼外漏｜詳見新章 §13）**：
> ①🔒**修樂手後台登入漏洞＋加審核制**：以前**任何** Firebase 帳號（含客戶 Gmail）都能進 `musician-dashboard`（只檢查 `if(!user)`）。加 gate：`musicians/{uid}` 須存在且 status∉{pending,rejected,suspended}（舊樂手/業主**無 status 欄位＝放行** grandfather；客戶沒 doc＝擋）。新流程：樂手在 `musician-login` 填完整申請（大頭照/Bio/學歷/經歷/作品連結/電話）→ `status:'pending'`、`published:false`、**不進後台只看「審核中」** → 管理者後台**「申請審核」分頁**核准(`status:'approved'`)/拒絕(+`rejectReason`)/停用 → 自動 email 通知申請人（`notifyMusicianOnReview`）。`musician-login` 登入也改成讀 `musicians/{uid}` 分流（非樂手→signOut＋提示；pending/rejected→狀態畫面）。
> ②📝**客戶 email＋密碼真帳號**：`customer-login` 的 email 流程以前是**假的**（只寫 localStorage、沒密碼沒真帳號，誰都能冒充）→ 改 `createUserWithEmailAndPassword`＋寄驗證信＋登入須已驗證（保留 Google）。順手修一個**潛藏 bug**：Facebook 鈕在 HTML 是註解掉的，但 JS 還 `btnFacebook.addEventListener`→對 null 拋錯→**中斷整支腳本→email/建帳號鈕全死**（加 `if(btnFacebook)` 守衛）。
> ③📜**平台政策系統（單一來源 `siteContent/policy`）**：管理者後台**「平台政策」**編輯各段（標題+內文+草擬註記，可增刪段）→ 音樂家後台**「平台政策」唯讀頁** ＋ 樂手註冊**同意書 modal** 都讀同一份（**改一次三處連動**）。樂手**「編輯主頁」前必須先到政策頁按「確認同意」**（gate：未 ack→導去政策頁；記 `policyAcknowledged/policyAckAt/policyAckVersion` 到樂手 doc）。預設 8 段草擬殼（收費抽成/教學/付款結算/取消退款/錄音影像權/行為準則/帳號審核停權/隱私）。
> ④📲**PWA（加到主畫面＝App）**：新增 `manifest.json`（`start_url=/musician-dashboard.html`、standalone）＋ iOS `apple-mobile-web-app` meta ＋ **方形紅底白螺旋** App 圖示（`icon-192/512.png`、`apple-touch-icon.png`、`favicon.png`，紅 `#741515`）。樂手 Safari/Chrome「加到主畫面」一次→桌面圖示一點全螢幕開後台、Firebase 持久登入＝免重打帳密。**故意不做 service worker**（怕舊快取）。⚠️ iOS 更新圖示要**移除舊捷徑重加**。
...
11. 🔴 **個資別存全域 localStorage key（會漏到別人）**：`media-sync.js` 會把 localStorage 媒體值同步到全站共用 `siteContent/media` 再還原給每台裝置。任何「沒有 uid 區隔的全域 key」一旦被它碰，就會散佈到**所有帳號**（曾兩度中招：`opusz_*` 客戶個資、`martin-profile-img` 樂手頭貼）。規則：顯示在公開/他人頁的資料只存「該帳號自己的 Firestore config（含 uid）」；個資 key 別讓 media-sync 同步。詳見 §13.5。
12. **curl 測線上 HTML 要加 `-L`**（Cloudflare Pages 把 `xxx.html` 308 轉址成 `/xxx`，不跟轉址會抓到空字串→誤判沒部署）。`media-sync.js`／`nav.js`／`nav.css` 等**外部腳本沒版本號＝瀏覽器吃舊快取，改了沒生效**；改完一定 bump `?v=N`（media-sync 目前 `?v=2`）。
...
0. ✅ **（已完成）放行管理者管理樂手權（Firestore 規則）** — `musicians/{uid}` 的 write 規則已是 `request.auth.uid == uid || request.auth.token.email == 'tzutung.liao@gmail.com'`（業主已發布）。所以後台**刪除/核准/拒絕/停用樂手**都能運作。
   - 🟡 **可選強化（未做，需業主發布）**：防樂手自我核准 —— 把 update 拆成「自己只能寫但不能改自己 `status`」：`allow update: if (request.auth.uid==uid && request.resource.data.status==resource.data.status) || request.auth.token.email=='tzutung.liao@gmail.com'`。現規則樂手理論上能改自己 status（要會寫程式才鑽得到，風險低）。詳見 §13.6。**Claude 不可代發，須業主貼上發布。**
...
## 13. 🆕 樂手審核制 / 客戶帳號 / 平台政策 / PWA（2026-06-09 新增）

### 13.1 樂手審核制（apply → review → approve）｜資料模型 `musicians/{uid}.status`
- **狀態值**：`pending`（新申請待審）→ `approved`（核准）／`rejected`（拒絕，另存 `rejectReason`）／`suspended`（停用）。**舊樂手＋業主可能沒有 `status` 欄位＝視為已核准（grandfather）**。
- **准入規則（兩個 gate，client 端）**：
  - `musician-dashboard.html` 的 `onAuthStateChanged`：讀 `musicians/{uid}`，**無 doc 或 status∈{pending,rejected,suspended}** → `signOut` + 導去 `musician-login.html?reason=...`。（這修掉了「任何登入帳號都能進後台」的漏洞。）
  - `musician-login.html` 的 `routeAfterAuth()`：登入/已登入時讀 status 分流（approved/active/無status→進後台；其餘→`step-status` 狀態畫面）。
- **申請表單**：`musician-login.html` 的 `doSetup()`（setup 步驟）收 大頭照(上傳 Storage `musicians/{uid}/photo_*`)＋Bio/學歷/經歷/連結/電話＋同意書勾選 → 寫 `musicians/{uid}`：`status:'pending', published:false, agreedToTerms, agreedAt, termsVersion`。送出後 signOut＋顯示「審核中」。
- **管理者審核**：`admin-panel.html` 側邊欄「**申請審核**」(`data-page="appreview"`)，`loadMusicianApplications()` 撈 `getDocs(musicians)` 篩 status∈{pending,approved,rejected} 顯示完整卡；`approveApplicant`(`{status:'approved', published:true}`)／`rejectApplicant`(prompt 理由→`{status:'rejected', rejectReason}`)／`suspendApplicant`。
- **email 通知**：核准/拒絕寫 doc → Cloud Function `notifyMusicianOnReview`（`functions/index.js`，onDocumentUpdated `musicians/{uid}`）寄信給申請人。
- ⚠️ **核准 ≠ 公開**：核准只設 `status:'approved'`（能進後台）＋`published:true`。但樂手仍須自己**編輯主頁→按發佈**才會出現在 `musicians.html`。公開列表(`musicians.html`/`lessons.html`)已加防呆：status∈{pending,rejected,suspended} 一律不顯示。

### 13.2 客戶 email＋密碼真帳號（`customer-login.html`）
- email 流程改 **Firebase `createUserWithEmailAndPassword` ＋ `sendEmailVerification`**；登入須 `emailVerified`（沒驗證會擋＋重寄）。建帳號 modal 有 email/密碼/確認密碼（CSS 同時涵蓋 `input[type=email/password]`）。Google 登入不變。
- 換不同 email 登入時，`handleUser` 會清掉前一帳號的本機個資（`opusz_city/intro/photo/bookings`）防殘留。
- ⚠️ 客戶頁面「登入判斷」用 localStorage(`opusz_user`)，但**訊息/conversations 需要真 Firebase session** —— 用假流程或舊資料建的客戶可能 localStorage 有登入但 Firebase 沒 session（訊息會無法用）。真帳號（email+密碼/Google）才完整。

### 13.3 平台政策系統｜單一來源 `siteContent/policy`（Firestore，admin 寫、公開讀）
- 結構：`{ draftNote, version, sections:[{title,body}], updatedAt }`。三處共用、**改一次全同步**：
  - **管理者編輯**：`admin-panel.html`「平台政策」(`data-page="policy"`)，`loadPolicyAdmin/polRender/polAddSection/polSave`（`setDoc(siteContent/policy)`）。
  - **樂手唯讀**：`musician-dashboard.html`「平台政策」(`data-page="policy"`)，`loadPolicy()`。
  - **註冊同意書**：`musician-login.html` 的「查看條款」modal（`openPolicyModal`）＋ 必勾才能送申請。
- 🔒 **編輯主頁前須先確認政策**：`musician-dashboard.html` 政策頁底部「確認同意」(`acknowledgePolicy()`→寫 `policyAcknowledged/policyAckAt/policyAckVersion`)；`navigate('pagebuilder')` 若未 ack→導去政策頁。內容全是「草擬中」佔位，業主之後自己填（數字先留白）。

### 13.4 PWA（加到主畫面＝App）
- `manifest.json`（根目錄，`start_url=/musician-dashboard.html`、`display:standalone`、紅 `#741515`）；圖示 `icon-192.png`/`icon-512.png`/`apple-touch-icon.png`/`favicon.png`（**方形紅底白螺旋**，用 PIL 把 logo 合成到實心紅方形產生）。
- `musician-dashboard.html`＋`musician-login.html` 的 `<head>` 加 `<link rel=manifest>`＋`apple-mobile-web-app-*` meta＋`apple-touch-icon`。
- **無 service worker**（刻意：避免舊快取害更新看不到）。「加到主畫面」要使用者**自己手動做一次**（iOS 不能自動裝）；改了圖示要**移除舊捷徑重加**才會更新。
- 想要「圖示紅點徽章/手機推播」要另做 **Web Push**（service worker＋通知權限＋伺服器推播，iOS 16.4+ 且須已安裝＋允許通知）——目前**只有 email 通知**，Web Push 未做。

### 13.5 🔴 頭貼/個資「漏到不相干帳號」的根因與斷根（務必記住）
- **症狀**：全新空帳號卻出現別人上傳過的頭貼/城市。
- **根因**：`media-sync.js` 把 localStorage 的 media 值同步到**全站共用 `siteContent/media`** 再還原給每個裝置；任何「無 uid 區隔的全域 key」一旦被它碰，就會散佈到所有人。中招的有：①`opusz_photo`/`opusz_city`…（客戶個資）②`martin-profile-img`（樂手檔案頭貼的舊 fallback）。
- **斷根做法**：①`media-sync.js` 排除所有 `opusz_` 開頭 key（restore/publish/migrate）＋全站 bump **`?v=2`**（沒版本號＝瀏覽器吃舊版，改了沒生效）。②移除 `musician-profile.html` 讀 `martin-profile-img` 的 `syncBarAvatar`，頭貼**只由各自 `config.about.avatarUrl`** 決定。
- **規則**：**顯示在公開/他人頁的資料，永遠別存在沒有 uid 區隔的全域 localStorage key；個資 key 別讓 media-sync 同步。** 雲端 `siteContent/media` 裡可能還躺著舊的 `martin-profile-img`/`opusz_*`，但已無程式讀取＝無害（要清需管理者寫 siteContent）。

### 13.6 Firestore 規則現況（與本章相關）
- `musicians/{uid}`：今已是 `read:true`、`write:` 自己(uid==uid) **或** 管理者(`tzutung.liao@gmail.com`) → **管理者審核/刪除已能運作**（不用再改規則）。
- `siteContent/{page}`（含 `policy`）：公開讀、管理者寫 → 政策編輯可運作。
- 🟡 **可選強化（未做，需業主發布）**：把 `musicians` 的 update 改成「自己只能寫但**不能改自己的 status**」防樂手自我核准——現規則樂手理論上能改自己 status（要會寫程式才鑽得到，風險低）。要做的話拆 create/update：`allow update: if (uid==uid && request.resource.data.status==resource.data.status) || 管理者email`。（Claude 不可代發，須業主貼上發布。）

---

## 14. 🆕 訊息「雙向即時聊天」打通 + 演出頁大改 + 一堆手機修正（2026-06-10 本視窗）

### 14.1 🔴🔴 訊息：客戶↔樂手「同一個聊天室、即時雙向」（本視窗最重要）
**背景**：之前是「兩套訊息」互不相通——客戶送的東西樂手收得到（`inquiries`），但客戶端聊天室（`conversations`）永遠空白、樂手回覆也不同步。本視窗全部打通。
- **三種直接聯繫**（musician-profile 的 直接訊息／檢視檔期／預約課程）：送出時 ①`opzSendInquiry` 寫 `inquiries`（給 Cloud Function 寄信＋樂手 inbox 備援）②`opzMirrorToConversation` 建立/寫入 **`conversations`**（雙向聊天室）。
- **客戶端** `customer-profile` Messages 讀 `conversations`（messaging.js `OPUSZ_MSG`）。
- **樂手後台「訊息」改成也讀/回 `conversations`**（不再讀 inquiries）→ 雙方同一聊天室、`onSnapshot` 即時雙向。
- **公開委托**（recent-jobs 發的）走**另一條**：→ `jobs` → 客戶 `customer-profile` 的 **My Bookings**（`OPUSZ_MSG.listMyJobs(uid)`）＋樂手後台「公開接案」看板。**不進聊天室**。
- **這次修掉 3 個疊在一起的隱藏雷**：
  1. 🐞 `opzMirrorToConversation` 抓不到客戶身分（`OPUSZ_MSG.currentUid()` 一時還 null）就**靜默放棄** → 改成 `OPUSZ_MSG.onAuth` 等身分就緒（最多 6s）才建。
  2. 🐞 **上課預約(lesson) 漏接鏡射** → 補上（booking/enquiry 本來就有）。
  3. 🐞🔴 **最深的雷**：`startConversation` 建立前先 `getDoc(ref)` 讀那筆對話，但對話**還不存在** → 讀取規則 `uid in resource.data.participants` 對 null 報錯 → **permission-denied** → 永遠建不出來。修法：`getDoc` 包 try/catch，**讀不到就當新對話直接建**（messaging.js）。

### 14.2 🔴 Firestore `conversations` 規則（業主已發布）＋「查詢必須符合規則」的超隱蔽雷
規則（`participants` = `[customerUid, musicianUid]`）：
```
...
```
- 🔴 **超隱蔽**：Firestore 只允許「查詢條件能保證符合讀取規則」的查詢。規則查 `uid in participants`，所以**列出對話一定要 `where('participants','array-contains', uid)`**；用 `where('musicianUid','==', uid)` 會被**整個查詢拒絕(permission-denied)**、收件匣全空。客戶端＋樂手後台**都已改 array-contains**。（這就是當初「客戶看得到、樂手空白」的原因。）
- `listConversations` 已**拿掉 orderBy**（client 端排序）→ **不必建複合索引**，只要上面這條規則。
- ⚠️ **規則「貼進編輯器」≠「已發布」**：本視窗一度卡在規則看起來在編輯器裡、實測卻 permission-denied——因為前一視窗貼了沒按發布。**改規則後一定要按「發布」**；Claude 不可代發。

### 14.3 §12.1 的更新（樂手後台 inbox 已換底）
§12.1 寫「樂手後台 訊息 = onSnapshot where musicianUid==我 讀 inquiries」——**本視窗已改**：`musician-dashboard.html` 的「訊息」現在讀/回 **`conversations`**（`participants array-contains`、`window._fsConvos`、`window.__listenConvMessages/__sendConvMessage/__markConvRead`）。`inquiries` 仍保留**只為 Cloud Function 寄信觸發**（onCreate 不變）。

### 14.4 客戶/樂手身分隔離（`opusz_role`）
- 之前漏洞：客戶區門禁只看 localStorage `opusz_user`，但**樂手後台登入也設這個** → 樂手一登入就能進客戶 `customer-profile`。
- 修法：登入貼角色標籤 `opusz_role`——樂手後台設 `'musician'`（登出清掉）；所有客戶登入點（customer-login、首頁快速登入 doLogin、recent-jobs pjOnAuthed）設 `'customer'`。`customer-profile` 開頭若 `opusz_role==='musician'` → 擋下導去 customer-login。**只擋明確 musician、真客戶一律放行（不誤鎖）**。

### 14.5 nav「My Account」全站一致（詳記在 §7 第 9a 點）
登入顯示 My Account、登出顯示 Log in，全站一致。**千萬別在 `nav.css` 加 `#navAccountBtn{display:none}`**（會壓過 wireAuth 清 inline 的顯示，所有 nav.js 頁面登入都看不到 My Account——本視窗被這條坑很久）。

### 14.6 演出頁（`shows.html`）大改
- **海報日期搬到上方 eyebrow**：原「01·2025」序號行改顯示 `show.N.date`；下方詳情移除日期行（剩 場地·曲目）。靜態 5 張 ftp-slide ＋ 動態 `_mkPosterDOM` 都改。
- **曲目/說明拆開**：`show.N.desc`=曲目（留詳情）；新增 `show.N.note`=說明 → 下方「瞭解更多」**可展開**（空的不顯示按鈕）。後台海報編輯器多了「說明」欄。
- **手機版**：①海報放大、文字上移靠近 ②「瞭解更多」改**彈出說明卡 modal**（不內嵌、不跳頁；點外面/×/Esc 關）——只手機(≤700)；桌機/平板維持內嵌。
- **Hero 標題在 MacBook/iPad/手機顯示**：原本 ≤1400px `visibility:hidden`（怕透過海報鏤空變鬼影）→ 改成顯示 + `onScroll` 內捲動淡出（只 ≤1400）避免鬼影；手機標題移上方、字級縮小。
- **🆕 Banner 濾鏡暗度滑桿**（後台可自調）：
  - 暗度 = `heroOverlay`（0–100，預設 88）→ `applyHeroOverlay` 設 `#heroOverlay` 漸層黑。
  - shows.html：`opzShows.setOverlay(v,save)` / `getOverlay()`。後台：`seBuildShowsHero`（admin-panel，sh-hero 分派）拖滑桿 → input 即時預覽、change 存檔（`saveState`+`syncShowsToServer`）。
  - 🐞 修：`loadState` 從 `shows-data.json` **漏讀 `heroOverlay`** → 線上永遠 88%。已補讀。
  - ⚠️ 這是**檔案型**設定（存 `shows-data.json`）→ 要在 **localhost 後台**調＋發布才存得進；線上後台只能預覽（模型 B，見 §2）。

### 14.7 手機 / 效能 修正雜項
- **樂手後台手機版大改**：左側黑側欄→**漢堡抽屜**（點 ☰ 滑出、半透明遮罩、點項目自動關）；間距收緊；訊息改手機聊天（列表→全螢幕對話＋返回鍵）。
- **聊天泡泡**：原本被擠成「每字一行」（`.bubble max-width:68%` 相對到 0 寬包裝層）→ 文字欄 ≤70%、`.bubble` 改 `fit-content`，像正常聊天 App；手機輸入框 `font-size:16px` 防 iOS 聚焦自動放大。
- **首頁 Hero 捲動動畫手機卡頓**：362 張逐格圖（記憶體＋主執行緒解碼）太重 → 手機(≤768)`_SCRUB_STEP` **只載 1/4 影格**、繪製就近對齊；桌機不變。
- **後台樂手列表/email**：列表讀錯欄位（資料是 `instruments`/`primaryRegion`，舊碼讀 `instrument`/`region`）→修；名字下加 email；樂手發布檔案時自動補寫 `email`。
- **新人導覽每次都跳**：自動跳出在 Firebase 載 uid 前就用 `anon` key 判斷「看過沒」→ 改成等 `window._tourUid` 就緒才判斷＋搬移舊 anon 記號。
- **首頁 footer**：電子報併進 footer 頂端（左訂閱、右四欄連結）；四欄撐滿邊到邊、行距加大；手機行距收緊。
- **移除 `messages.html`**：舊獨立客戶訊息頁（示範假資料）退役 → 所有「My Account→Messages」連結改指 `customer-profile.html#messages`（已支援 hash 自動開分頁）；`nav.js`/`nav-auth.js` 版本號 bump。
- 移除 `lessons.html` hero 的「Music Lessons」小標籤。

### 14.8 🔴 後台 Firebase 登入會過期（業主常踩，務必記住）
- `admin-panel.html` 門禁只看 localStorage `opusz_admin_loggedIn`（**不會過期**），但**真正的 Firebase 登入會過期**。失效後：`musicians`（公開讀）還看得到、但 `customers`/`jobs`（要登入才能讀）會 **permission-denied** → 後台「Failed to load customers」。
- **解法**：登出 → `admin-login.html` 用管理者帳號重新登入（建立新 Firebase session）。⚠️ **坑**：登入頁會被那個 localStorage 旗標彈回後台、進不去 → 先清掉 `opusz_admin_loggedIn`（或登出鈕）才到得了登入頁。
- 🟡 **建議未來改**（未做）：`admin-panel` 加 `onAuthStateChanged`，Firebase 一過期就自動導回 `admin-login`，免得旗標顯示「已登入」其實沒連線。

### 14.9 本視窗動到的快取版本號（改了要再 bump）
- `nav.js` 與 `nav.css`、`messaging.js?v=`、`nav-auth.js?v=` 本視窗都動過（**看各檔當前值**）。**外部 `<script>/<link>` 改了，硬重新整理常刷不到，版本號才可靠**；HTML 沒版本號靠 Cloudflare 服務最新，但瀏覽器可能要硬重整。
